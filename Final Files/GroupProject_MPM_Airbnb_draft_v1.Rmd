...
# Fitting a GAM to non-linear variable 'accommodates'
...
# Applying the Support Vector Machines
...
As the Possion, Binomial and SVM Models do not apply to our data set and our response variable, we will not consider their values. These have to interpreded individually and should be compared in their group only. Additionally, we have to mention that the ANN was only trained with a small subset of the data set. Our resources did not allow to run a callculation on the complete data set. Hence, the result could vary if the data is trained with all available data.
