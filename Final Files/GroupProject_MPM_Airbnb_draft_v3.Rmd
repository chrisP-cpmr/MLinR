...
From all the other models, we can see that the Neural Network has the lowest RMSE and would therefore be our final model.
You will find the model and the calculations of the Neural Network in a separate file named accordingly. The reason for this is, that the project took to long to Knit with the Neural Network calculations included.
...
As a result we would suggest to use the linear model from a business perspective.
The reason being, a Neural Network will not produce a result in the desired time which a user would wait. Hence, we prefer a simpler, less accurate but quicker model.

## Leassons learned

During this project we ran into several challenges which:

- Different models need different types of raw data, for example, categorical values as integers or strings
- Some packages could not be installed (plgraphics)
- Neural Networks are accurate but take a lot of computing power
- Applying all the models with each predictor is an extensive process
- GITHub was not as simple to use and produced merging errors
- Knitting often
